...
const Symptomchk = () => {
 ...
 // const [FsymptomArray, setFSymptomsArray] = useState("");
  ...
  const stringtoarray=(e)=>{
    const input = e.target.value;
    const symptomsArray = input.split(',').map(symp => symp.trim().replace(/\s+/g, "_"));
    const transformedString = symptomsArray.join(',');
  ...
  const handlePrediction = async (e) => {
    ...
    try {
      ...
      //console.log("here is res",(response.data.output[0]));
      let jsonString = response.data.output[0];
