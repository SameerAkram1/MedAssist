...
# Loading datasets
sym_des = pd.read_csv(r"D:\AI-MedLab-main\AI-MedLab\backend\HealthPredict\symtoms_df.csv")
precautions = pd.read_csv(r"D:\AI-MedLab-main\AI-MedLab\backend\HealthPredict\precautions_df.csv")
workout = pd.read_csv(r"D:\AI-MedLab-main\AI-MedLab\backend\HealthPredict\workout_df.csv")
description = pd.read_csv(r"D:\AI-MedLab-main\AI-MedLab\backend\HealthPredict\description.csv")
medications = pd.read_csv(r"D:\AI-MedLab-main\AI-MedLab\backend\HealthPredict\medications.csv")
diets = pd.read_csv(r"D:\AI-MedLab-main\AI-MedLab\backend\HealthPredict\diets.csv")

# Load the trained model
with open(r"D:\AI-MedLab-main\AI-MedLab\backend\ai-models\svc.pkl", 'rb') as model_file:
    model = pickle.load(model_file)

# Predict disease based on input symptoms
def get_predicted_value(arguments):
    ...
    predicted_disease_id = model.predict([input_vector])[0]
    return diseases_list.get(predicted_disease_id, "Unknown Disease")



def helper(disease):
    desc = description[description['Disease'] == disease]['Description'].values[0]
    pre = precautions[precautions['Disease'] == disease][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values.flatten().tolist()
    
    # Get medications and diets as lists directly
    ...
    return {
        "description": desc,
        "precautions": pre,
        "medications": med,  # Ensure this is a flat list
        "diet": die,  # Ensure this is a flat list
        "workout": wrkout
    }



# Main function to get prediction and additional info
def predict_disease_and_info(arguments):
    #print("Symptoms received:", symptoms)
    predicted_disease = get_predicted_value(arguments)
    if predicted_disease == "Unknown Disease":
        return json.dumps({"error": "Could not predict the disease."})
...
if __name__ == "__main__":
    print(predict_disease_and_info(arguments))
